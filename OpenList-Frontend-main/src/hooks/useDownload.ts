...
export const useDownload = () => {
  ...
  return {
    ...
    sendToAria2: async () => {
      ...
      const fetchFolderStructure = async (
        ...
        if (!obj.is_dir) {
          ...
          // 处理租户端路径
          let path = pathJoin(pathname(), pre, obj.name);
    ...
    playlistDownloadSelected: () => {
      ...
      // 处理租户端路径
      let saveNamePath = pathname();
      if (saveNamePath.startsWith("/@tenant/data")) {
        // 租户端路径需要转换为实际路径
        saveNamePath = saveNamePath.substring("/@tenant/data".length) || "/";
      }
      let saveName = pathBase(saveNamePath)
