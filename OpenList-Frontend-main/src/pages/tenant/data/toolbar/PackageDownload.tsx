...
const PackageDownload = (props: { onClose: () => void }) => {
  ...
  const fetchFolderStructure = async (
    ...
    if (!obj.is_dir) {
      ...
      let fullPath = pathJoin(pathname(), pre, obj.name)
      if (fullPath.startsWith("/@tenant/data")) {
        // 租户端路径需要转换为实际路径
        fullPath = fullPath.substring("/@tenant/data".length) || "/";
      }
      const resp = await fsList(fullPath, password())
      if (resp.code !== 200) {
        return resp.message
      }
      const res: File[] = []
