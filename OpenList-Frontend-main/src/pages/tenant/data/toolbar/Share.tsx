...
import { batch, createSignal, Match, onCleanup, Switch as SolidSwitch } from "solid-js"
import {
  ExtractFolder,
  OrderBy,
  OrderDirection,
    ...
    if (name === "share") {
      batch(() => {
        ...
        const paths = selectedObjs().map((obj) => {
          ...
          const split =
            path.endsWith("/") || obj.name.startsWith("/") ? "" : "/"
          return `${me().base_path}${path}${split}${obj.name}`
        })
        setShare({
          files: paths,
          expires: null,
              ...
              <Button
                ...
                onClick={async () => {
                  ...
                  handleResp(resp, (data) => {
                    setLink(
                      `${location.origin}${matchTemplate(
                        getSetting("share_template"),
